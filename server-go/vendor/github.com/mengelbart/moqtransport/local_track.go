...
type LocalTrack struct {
	...
	subscribers        map[subscriberID]*sendSubscription
...
func (t *LocalTrack) loop() {
	...
	for {
		...
		case object := <-t.objectCh:
			for _, v := range t.subscribers {
				if v.filterType == wire.FilterTypeLatestGroup && !v.firstObjectSent && object.ObjectID != 0 {
					continue
				}
				...
				if v.filterType == wire.FilterTypeLatestGroup && !v.firstObjectSent {
					v.firstObjectSent = true
				}
			}
